...
void PointCloud2Msg::setDataMembers(const std::shared_ptr<Camera> &camera,
    ...
    uint16_t *minRange =
        std::min_element(frameData, frameData + frameHeight * frameWidth);
    uint16_t *maxRange =
        std::max_element(frameData, frameData + frameHeight * frameWidth);
    ...
    for (int i = 0; i < frameHeight; i++) {
        for (int j = 0; j < frameWidth;
            ...
            // mark the points corresponding to the min and max range as invalid
            if (depth == *minRange || depth == *maxRange) {
                ...
                *iter_rgb = frameData[index] +
                            frameData[frameHeight * frameWidth + index];
            }
        }
    }
}
